...
\begin{compactitem}
  ...
  \item \textbf{Compatible incentive}: The incentives for different players, either in competitive or collaborative games, should be compatible such that neither sides have shortcuts and could play in a fair setting. This is mainly reflected in the bonus players receive.
\end{compactitem}
...
This is the base game where the player directly labels document sentiment and selects words that influence decision making (informative words).
...
While in \emph{Hangman-Subtract}, each masked word is automatically assigned a equal cost, the masker can manually allocate a fixed number of budget to all the words masked, and the guesser will need to pay the cost accordingly to unmask the word. 
