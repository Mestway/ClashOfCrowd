...
In the last milestone, we refined our research questions and sketched our game mockups. Till now, we have implemented our game platform (for coordinating players to start coop games) and the first three games (DirectAnnotation, Bingo and HangmanAdd). Furthermore, we designed the data-analysis method that will be used to analyze experiment result.

Remaining tasks include (1) implement the two games left, (2) deploy all five games to the crowds, and (3) analyze data collected from the experiments.
